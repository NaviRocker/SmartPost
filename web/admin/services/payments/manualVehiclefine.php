<?php
    ...
    if(mysqli_connect($host,$Hname,$Hpwd,$dbname)){
        ...
        if(!empty($_POST['vehicleNo']) && !empty($_POST['address']) && !empty($_POST['date']) && !empty($_POST['DLNo']) && !empty($_POST['PStation']) && !empty($_POST['NIC'])&& !empty($_POST['amount']) && !empty($_POST['method'])) {
            ...
            $query = "INSERT INTO manual_vehiclefine (vehicleNo, address, date, DLNo, PStation, NIC, amount, method, refNo, cashCollected) VALUES ('$vehicleNo', '$address', '$date', '$DLNo', '$PStation', '$NIC', '$amount', '$method', '$refNo', '$cashCollected')";        
